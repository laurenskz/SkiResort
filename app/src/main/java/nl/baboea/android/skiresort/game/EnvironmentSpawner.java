...
public class EnvironmentSpawner implements ParticipantContainer {
    ...
    private boolean alreadyGenerated(Vec3 gridPoint) {
        ...
        if (lastTile.equals(gridPoint, 0.1f))
            return true;//Player is in the last tile majority of time.
        for (Vec3 generated : pointsAtGenerated) {
            if (gridPoint.equals(generated, 0.1f))
                return true;//Second parameter is epsilon. We don't care really much for precision here
        }
        return false;
    }
    ...
    private void cleanUpPoints() {
        ...
        while (iterator.hasNext()) {
            ...
            if (point.getY() - GENERATION_HEIGHT * 1.5 > Camera.getPosition().getY() + (1f / Constants.RATIO))
                iterator.remove();
        }
    }
